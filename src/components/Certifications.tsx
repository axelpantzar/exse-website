...
export function Certifications() {
  ...
  return (
    <section className="mt-28 overflow-hidden rounded-3xl bg-foreground p-8 text-background md:p-14">
      <div className="grid gap-10 md:grid-cols-12 md:items-end">
        ...
        <Reveal as="p" delay={120} className="text-lg text-background/80 md:col-span-7">
          {t({
      ...
      <ul className="mt-12 grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-5">
        {items.map((c, i) => (
          ...
          >
            <div className="flex h-24 w-24 items-center justify-center rounded-2xl bg-background/5 p-3 md:h-28 md:w-28">
              <img
                src={c.src}
                alt={c.label}
                loading="lazy"
                className="max-h-full max-w-full object-contain brightness-0 invert opacity-80 transition-all duration-300 hover:opacity-100"
              />
            </div>
            <p className="mt-4 text-xs leading-snug text-background/70">{c.label}</p>
          </Reveal>
        ))}
      </ul>
    </section>
